...
# The Service 
The service layer contains all the business logic of the application. Its responsibility is to enforce business rules, perform validation, coordinate operations across the persistence layer, and ensure consistent behaviour regardless of how the application is accessed. Both the console application and the REST API delegate their requests to the same service methods, which means the business logic is implemented only once and reused across multiple interfaces. This separation keeps controllers and the console lightweight, improves testability, and makes the application easier to maintain and extend.
